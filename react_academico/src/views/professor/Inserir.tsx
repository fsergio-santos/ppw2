...
import useInsertProfessor from '../../service/connection/professor/InsertProfessor';
import {
  BUTTON_SIZE,
...
const navegacaoUsuario = {
  ...
  descricao: `Incluir novo ${PROFESSOR_SHOW} no sistema`,
...
const Incluir = () => {
  ...
  const { insertProfessor, errorInsertProfessor } = useInsertProfessor();
  ...
  const onSubmitData = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const data = await insertProfessor(model);
    ...
    if (data && data.dados) {
      ...
      setErrors(errorInsertProfessor);
      showAlert(MENSAGEM_SISTEMA.PROFESSOR.DANGER, DANGER, TIME);
    }
  };
